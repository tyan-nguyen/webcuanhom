...
BulkButtonWidget::widget([
    ...
    Html::a('<i class="fa-solid fa-square-plus"></i> Thêm mới (A)', ['create'],
        ['role'=>'modal-remote','title'=> 'Thêm mới thiết bị','class'=>'btn btn-primary btn-sm btn-default-custom', 'accesskey'=>'a']).
        '&nbsp;' .
    ...
    Html::a('<i class="fa-solid fa-upload"></i> Upload', Yii::getAlias('@web/dungchung/import-all/upload?type=').ThietBi::MODEL_ID . '&showOverwrite=1', [ 'class'=>'btn btn-primary btn-sm btn-default-custom', 'role'=>'modal-remote']) . '&nbsp;' .
    
    Html::a('<i class="fa-solid fa-cloud-arrow-down"></i> Tải mẫu', Yii::getAlias('@web/files/excel/import-thiet-bi.xlsx'), ['class'=>'btn btn-primary btn-sm btn-default-custom'])
]);
?>
</div>
...
<div class="kho-vat-tu-index">
    <div id="ajaxCrudDatatable">
        <?=GridView::widget([
            ...
            'panel' => [
                'heading' => '<i class="fa-brands fa-windows"></i> Danh sách kho thiết bị'
            ]
        ])?>
    </div>
</div>
